...
# Add ecosystem legend if shown
if show_ecosystem:
    legend_items.append(f'<i style="background:orange; width:10px; height:10px; float:left; margin-right:6px;"></i> Ecosystem Sites<br>')

# Add farmers market legend if shown
if show_farmers:
    for support_type, color in dcase_color.items():
        if support_type in selected_dcase:
            legend_items.append(f'<i style="background:{color}; width:10px; height:10px; float:left; margin-right:6px;"></i> {support_type}<br>')

# Add SNAP legend if shown
if show_snap:
    for store_type in selected_store_types:
        color = snap_colors.get(store_type, "black")
        legend_items.append(f'<i style="background:{color}; width:10px; height:10px; float:left; margin-right:6px;"></i> {store_type}<br>')

# Add tavern types legend if shown
if show_taverns:
    for license_type in selected_tavern_types:
        color = license_colors.get(license_type, "black")
        legend_items.append(f'<i style="background:{color}; width:10px; height:10px; float:left; margin-right:6px;"></i> {license_type}<br>')

# Only create legend if something is selected
if legend_items:
    legend_html = f"""
    <div style="
        position: fixed;
